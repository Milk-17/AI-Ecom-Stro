...
import { PencilOff, Delete, Box, ChevronLeft, ChevronRight } from "lucide-react"; // เพิ่ม Icon ลูกศร
...
const FormProduct = () => {
  ...
  const filteredProducts = products.filter((item) => {
      ...
      const matchesCategory = categoryFilter 
        ? item.categoryId === Number(categoryFilter) || item.subCategoryId === Number(categoryFilter)
        : true;
  ...
  return (
    <div className="container mx-auto p-6 bg-white shadow-lg rounded-lg border border-gray-200">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col space-y-6 mb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
                ...
                <input className="border border-gray-300 p-2.5 rounded-md w-full focus:ring-2 focus:ring-blue-500 outline-none transition"
                    onChange={handleOnChange} value={form.title} name="title" placeholder="Product Title" required />
            </div>
            <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">รายละเอียด</label>
                <textarea className="border border-gray-300 p-2.5 rounded-md w-full h-[46px] resize-none focus:ring-2 focus:ring-blue-500 outline-none overflow-hidden transition"
                    onChange={handleOnChange} value={form.description ?? ""} name="description" placeholder="Description" />
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
                ...
                <input className="border border-gray-300 p-2.5 rounded-md w-full focus:ring-2 focus:ring-blue-500 outline-none transition"
                    type="number" onChange={handleOnChange} value={form.price} name="price" placeholder="0.00" min="0" required />
            </div>
            <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">จำนวน (ชิ้น)</label>
                <input className="border border-gray-300 p-2.5 rounded-md w-full focus:ring-2 focus:ring-blue-500 outline-none transition"
                    type="number" onChange={handleOnChange} value={form.quantity} name="quantity" placeholder="0" min="0" required />
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
                ...
                <select className="border border-gray-300 p-2.5 rounded-md w-full focus:ring-2 focus:ring-blue-500 outline-none bg-white cursor-pointer"
                    value={selectedMainId}
                    onChange={(e) => { setSelectedMainId(e.target.value); setForm({ ...form, categoryId: "" }); }} required >
            ...
            <div className="space-y-2">
                ...
                <select className="border border-gray-300 p-2.5 rounded-md w-full focus:ring-2 focus:ring-blue-500 outline-none bg-white cursor-pointer disabled:bg-gray-100"
                    value={form.categoryId} name="categoryId" onChange={handleOnChange} disabled={!selectedMainId} required >
      ...
      <ProductFilterHeader 
        ...
        handleReset={() => { setSearch(""); setCategoryFilter(""); }}
      />
          ...
          <tbody className="text-sm text-gray-600">
            {currentProducts.length > 0 ? (
                currentProducts.map((item, index) => {
                    ...
                    return (
                        <tr key={index} className={`border-b last:border-0 hover:bg-gray-50 transition duration-150 ${isOutOfStock ? "bg-red-50" : ""} ${isLowStock ? "bg-yellow-50" : ""}`}>
                            <td className="p-3 text-center align-middle">{realIndex}</td>
                            <td className="p-3 text-center align-middle">
                                <div className="flex justify-center">
                                    {item.images.length > 0 ? (
                                        <img className="w-12 h-12 rounded-md shadow-sm object-cover border border-gray-200" src={item.images[0].url} alt={item.title} />
                                    ) : (
                            ...
                            <td className="p-3 text-right align-middle font-mono font-semibold text-gray-700">{numberFormat(item.price)}</td>
                            <td className="p-3 text-center align-middle">{getStockStatus(item.quantity)}</td>
                            <td className="p-3 text-center align-middle font-medium">{item.sold}</td>
